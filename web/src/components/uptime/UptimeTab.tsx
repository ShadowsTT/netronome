...
import { UptimeMonitor, UptimeMonitorInput, UptimeUpdate } from "@/types/types";
import {
  createUptimeMonitor,
  deleteUptimeMonitor,
  getUptimeMonitors,
...
import { UptimeMonitorDetails } from "./UptimeMonitorDetails";
import { defaultUptimeFormData, describeTarget, stateBadge } from "./constants";

// statusText is the short live figure on a monitor card.
const statusText = (status: UptimeUpdate): string => {
  if (status.success) {
    return `${(status.responseTimeMs ?? 0).toFixed(1)} ms`;
  }
  return status.statusCode ? `HTTP ${status.statusCode}` : "failed";
};

export const UptimeTab: React.FC = () => {
  const queryClient = useQueryClient();
  const [selectedId, setSelectedId] = useState<number | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [editingMonitor, setEditingMonitor] = useState<UptimeMonitor | null>(
    null,
  );
  const [formData, setFormData] = useState<UptimeMonitorInput>(
    defaultUptimeFormData,
  );
  const [monitorToDelete, setMonitorToDelete] = useState<UptimeMonitor | null>(
    null,
  );
  ...
  // Live status: the server has no event stream for uptime checks, so each
  // enabled monitor polls its own status route.
  const statuses = useQueries({
    queries: monitors
      .filter((monitor) => monitor.enabled)
      .map((monitor) => ({
        queryKey: ["uptime", "status", monitor.id],
        queryFn: () => getUptimeMonitorStatus(monitor.id),
        refetchInterval: 5000,
      })),
    combine: (results) =>
      new Map<number, UptimeUpdate>(
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (monitor) => {
      ...
      showToast("Uptime monitor created", "success", {
        description: `Monitoring ${monitor.target}`,
      });
      handleCancelForm();
    },
    onError: (error: Error) => {
      showToast("Failed to create uptime monitor", "error", {
  ...
  const updateMutation = useMutation({
    mutationFn: updateUptimeMonitor,
    onSuccess: (monitor) => {
      void refreshMonitors();
      ...
      if (editingMonitor) {
        showToast("Uptime monitor updated", "success", {
          description: monitor.name || monitor.target,
        });
        handleCancelForm();
      }
    },
    onError: (error: Error) => {
      showToast("Failed to update uptime monitor", "error", {
        description: error.message,
      });
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteUptimeMonitor,
    onSuccess: (_, monitorId) => {
      void refreshMonitors();
      if (selectedId === monitorId) {
        setSelectedId(null);
      }
      showToast("Uptime monitor deleted", "success");
    },
    onError: (error: Error) => {
      showToast("Failed to delete uptime monitor", "error", {
        description: error.message,
      });
    },
  });

  const handleSubmit = (data: UptimeMonitorInput) => {
    if (editingMonitor) {
  ...
  const handleEdit = (monitor: UptimeMonitor) => {
  ...
  const handleCancelForm = () => {
    ...
    setFormData(defaultUptimeFormData);
  };
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 md:items-start">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">
                HTTP endpoints and TCP ports, checked on a schedule
              </p>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                No uptime monitors configured yet
              </p>
              <p className="text-gray-500 dark:text-gray-500 text-sm mt-2">
                Add a monitor to watch a URL or a port
              </p>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-gray-600 dark:text-gray-400 text-sm mb-2 truncate font-mono">
                          {describeTarget(monitor)}
                        </p>
                      </div>
      ...
      <DeleteMonitorModal
        ...
        monitor={
          monitorToDelete && {
            name: monitorToDelete.name,
            host: monitorToDelete.target,
          }
        }
      />
    </div>
