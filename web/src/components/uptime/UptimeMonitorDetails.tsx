...
import { UptimeMonitor, UptimeUpdate } from "@/types/types";
import { getUptimeHistory } from "@/api/uptime";
import { MetricCard } from "@/components/common/MetricCard";
import { formatters, formatDateTimeWithSettings } from "@/utils/timeSettings";
import { daysUntil, stateBadge } from "./constants";

interface UptimeMonitorDetailsProps {
  monitor: UptimeMonitor;
  status?: UptimeUpdate;
}

// A certificate this close to expiry is worth a warning colour.
const CERT_WARN_DAYS = 14;

export const UptimeMonitorDetails: React.FC<UptimeMonitorDetailsProps> = ({
  monitor,
  status,
}) => {
  const {
    ...
    queryKey: ["uptime", "history", monitor.id],
    queryFn: ({ pageParam }) => getUptimeHistory(monitor.id, pageParam),
    initialPageParam: 1,
  ...
  const statusCode = status ? status.statusCode : latest?.statusCode;
  const certExpiry = status ? status.certExpiry : latest?.certExpiry;
  ...
  const isHTTPS = monitor.type === "http" && monitor.target.startsWith("https://");
  const certDays = certExpiry ? daysUntil(certExpiry) : undefined;
    ...
    >
      <div className="bg-gray-50/95 dark:bg-gray-850/95 rounded-xl p-6 shadow-lg border border-gray-200 dark:border-gray-800">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
          {monitor.name || monitor.target} Details
        </h2>

        <div
          className={`grid grid-cols-1 sm:grid-cols-3 ${
            isHTTPS ? "lg:grid-cols-4" : ""
          } gap-4 mb-6`}
        >
          <MetricCard
          ...
          <MetricCard
            ...
            title={monitor.type === "tcp" ? "Port" : "Status"}
            value={
              monitor.type === "tcp"
                ? success === undefined
                  ? "—"
            ...
            unit=""
            status={success === false ? "error" : "normal"}
          />
          <MetricCard
          ...
          {isHTTPS && (
            <MetricCard
              ...
              title="Cert expires"
              value={certDays === undefined ? "—" : String(certDays)}
              unit={certDays === undefined ? "" : "days"}
              status={
                certDays === undefined
                  ? "normal"
                  : certDays < 0
                    ? "error"
                    : certDays <= CERT_WARN_DAYS
                      ? "warning"
                      : "normal"
              }
            />
